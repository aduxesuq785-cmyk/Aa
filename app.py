...
PAGES = {
    "/user.html": ("store", "text/html; charset=utf-8"),
    "/admin.html": ("admin", "text/html; charset=utf-8"),
}
ALIASES = {
    "/shop": "/user.html",
    "/store": "/user.html",
...
def page_landing():
    """Tiny index that links both apps."""
    return LANDING_HTML
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        self.route(want_head=True)

    def log_message(self, fmt, *args):
        sys.stderr.write("[bot] %s\n" % (fmt % args))
...
def main():
    try:
        ...
        print("cannot bind %s:%s -> %s" % (HOST, PORT, exc), file=sys.stderr)
        return 1
    print("bot.py serving on http://%s:%s" % (HOST, PORT))
    print("  /           landing")
    print("  /user.html  store      (%d bytes)" % len(USER_HTML.encode("utf-8")))
    print("  /admin.html admin      (%d bytes)" % len(ADMIN_HTML.encode("utf-8")))
...
<head>
  ...
  <style>
    * {
    ...
    .pm-section-label { color: #00ffff; font-size: 14px; margin: 18px 0 10px; }
    .pm-tiles { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 10px; }
    .pm-tile { border: 2px solid rgba(255,255,255,.14); background: rgba(255,255,255,.04);
               color: #e5e7eb; border-radius: 12px; padding: 12px 10px; cursor: pointer;
               display: flex; flex-direction: column; gap: 6px; align-items: center;
               transition: .18s ease; font: inherit; }
    .pm-tile:hover { transform: translateY(-2px); border-color: rgba(0,255,255,.5); }
    .pm-tile.sel { border-color: #00ffff; background: rgba(0,255,255,.10);
                   box-shadow: 0 0 20px rgba(0,255,255,.22); }
    .pm-tile-ic { font-size: 19px; }
    .pm-tile-nm { font-size: 13.5px; font-weight: 700; }
    .pm-tile-no { font-size: 11px; color: #94a3b8; word-break: break-all; }
    .pm-detail { border: 1px solid rgba(0,255,255,.25); border-radius: 12px; padding: 14px;
                 margin-top: 12px; background: rgba(0,0,0,.28); text-align: center; }
    .pm-num { display: flex; align-items: center; justify-content: center; gap: 10px;
              font-size: 18px; font-weight: 800; color: #00ff88; letter-spacing: .04em;
              margin: 8px 0; flex-wrap: wrap; }
    .pm-copy { background: rgba(0,255,255,.1); border: 1px solid rgba(0,255,255,.4);
               color: #00ffff; border-radius: 8px; padding: 5px 10px; font-size: 11px; cursor: pointer; }
...
<button class="filter-btn" data-filter="free" onclick="filterProducts('free')">
  ...
  <div id="checkoutModal" class="modal">
    <div class="modal-content">
      ...
      <div class="btn-group">
        ...
        <button class="btn" id="proceedToPaymentBtn" onclick="proceedToPayment()">
  ...
  <div id="paymentModal" class="modal">
    <div class="modal-content">
      ...
      <div class="pm-section-label"><i class="fas fa-wallet"></i> Select Payment Method</div>
      <div id="paymentMethodsWrap" class="pm-tiles"></div>

      <div class="pm-detail" id="pmDetail" style="display:none;">
        <div id="pmDetailTitle" style="color:#999;font-size:12px;">Send / Scan to pay</div>
        <div class="pm-num"><span id="pmDetailNumber">-</span>
          <button type="button" class="pm-copy" onclick="copyPaymentNumber()">
            <i class="fas fa-copy"></i> Copy
          </button>
        </div>
        <img class="qr-image" id="pmDetailQr" alt="Payment QR" style="display:none;">
        <p class="pm-info" id="pmDetailInstructions"></p>
      </div>

      <img class="qr-image" src="https://i.ibb.co/RTNBtWmc/file-000000004df8724699e53cff0d1f183c.png" alt="QR Code">
      <p class="info-text" id="paymentHintText">
        <i class="fas fa-info-circle"></i> Use the selected method above (Send Money or scan the QR), then enter the transaction details below to confirm your order.
      </p>
      ...
      <div class="input-group">
        ...
        <small style="color: #999; font-size: 12px; margin-top: 5px; display: block;">
          Enter the exact amount you paid via your mobile wallet or bank transfer
        </small>
      </div>

      <div class="input-group">
        <label><i class="fas fa-receipt"></i> Transaction ID / UTR <span id="utrOptionalTag" style="color:#999;font-size:11px;"></span></label>
        <input type="text" id="utrId" placeholder="Enter 10-16 character Transaction ID" maxlength="16">
        <small style="color: #999; font-size: 12px; margin-top: 5px; display: block;">
      ...
      <div class="btn-group">
        ...
        <button class="btn" id="confirmOrderBtn" onclick="confirmPurchase()">
...
function applyFilter(products, filter) {
  switch(filter) {
    ...
    async function confirmPurchase() {
      ...
      const selectedMethod = window.selectedPaymentMethod || null;
      let trxRequired = true;
      if (selectedMethod && typeof selectedMethod.trxRequired === 'boolean') {
        trxRequired = selectedMethod.trxRequired;
      } else if (window.storePaymentConfig && typeof window.storePaymentConfig.requireTrxId === 'boolean') {
        trxRequired = window.storePaymentConfig.requireTrxId;
      }

      if (trxRequired) {
        if (!utrId) {
          showNotification(selectedMethod
            ? 'Please enter the Transaction ID from your ' + (selectedMethod.name || '') + ' payment'
            : 'Please enter UTR / Transaction ID', 'error');
          return;
        }
        if (utrId.length < 10 || utrId.length > 16) {
          showNotification('Transaction ID must be between 10 and 16 characters', 'error');
          return;
        }
        if (!/^[a-zA-Z0-9]+$/.test(utrId)) {
          showNotification('Transaction ID must contain only letters and numbers', 'error');
          return;
        }
      }
      ...
      try {
        ...
        for (const item of window.cart) {
          ...
          const orderData = {
            ...
            paymentMethodKey: selectedMethod ? selectedMethod.__key : 'qr',
            paymentMethod: selectedMethod ? (selectedMethod.name || selectedMethod.__key) : 'QR / Bank transfer',
            productSnapshot: {
              title: item.title,
              imageUrl: item.image,
    ...
    function pmSortKey(m) {
      return typeof m.sortOrder === 'number' && isFinite(m.sortOrder) ? m.sortOrder : 999;
    }

    async function loadStorePaymentMethods() {
      try {
        const snap = await window.dbGet(window.dbRef(window.db, 'meta/paymentMethods'));
        const cfg = await window.dbGet(window.dbRef(window.db, 'meta/paymentConfig'));
        window.storePaymentConfig = cfg.exists() ? (cfg.val() || {}) : {};
        const raw = snap.exists() ? (snap.val() || {}) : {};
        paymentMethods.length = 0;
        Object.keys(raw).forEach(function (k) {
          const m = raw[k] || {};
          if (m.enabled === false) return;
          paymentMethods.push(Object.assign({ __key: k }, m, { id: m.id || k }));
        });
        paymentMethods.sort(function (a, b) {
          return pmSortKey(a) - pmSortKey(b);
        });
      } catch (error) {
    ...
    function markLegacyQr(show) {
      const legacy = document.querySelector('#paymentModal img[src*="i.ibb.co"]');
      if (legacy) legacy.style.display = show ? 'block' : 'none';
    }
    ...
    function renderPaymentMethodTiles() {
      ...
      if (!paymentMethods.length) {
        wrap.innerHTML = '<div class="pm-empty"><i class="fas fa-triangle-exclamation"></i> ' +
          (paymentMethodsLoaded
            ? 'No payment method is enabled right now. Use the QR code below, or contact support.'
            : 'Loading payment options...') + '</div>';
        const d = document.getElementById('pmDetail');
        if (d) d.style.display = 'none';
        const hint0 = document.getElementById('paymentHintText');
        if (hint0) hint0.textContent = 'Scan the QR code and complete payment, then enter your details below.';
        markLegacyQr(true);
        return;
      }
      markLegacyQr(false);

      wrap.innerHTML = paymentMethods.map(function (m) {
        const color = m.color || '#00ffff';
        const icon = m.iconCls || 'fa-wallet';
        const sel = window.selectedPaymentMethod && window.selectedPaymentMethod.__key === m.__key;
        return '<button type="button" class="pm-tile' + (sel ? ' sel' : '') + '" data-pm="' +
          escapeHtml(m.__key) + '">' +
          '<span class="pm-tile-ic" style="color:' + color + '"><i class="fas ' + escapeHtml(icon) + '"></i></span>' +
          '<span class="pm-tile-nm">' + escapeHtml(m.name || m.__key) + '</span>' +
          (m.number ? '<span class="pm-tile-no">' + escapeHtml(m.number) + '</span>' : '') +
          '</button>';
      }).join('');

      wrap.querySelectorAll('button.pm-tile').forEach(function (tile) {
        tile.addEventListener('click', function () {
          selectPaymentMethod(tile.getAttribute('data-pm'));
        });
      });

      const stillValid = window.selectedPaymentMethod &&
        paymentMethods.some(function (m) { return m.__key === window.selectedPaymentMethod.__key; });
      const chosen = stillValid
        ? window.selectedPaymentMethod
        : (paymentMethods.find(function (m) { return m.isDefault; }) || paymentMethods[0]);
      selectPaymentMethod(chosen.__key);
    }

    function selectPaymentMethod(key) {
      const m = paymentMethods.find(function (x) { return x.__key === key; });
      if (!m) return;
      window.selectedPaymentMethod = m;
      document.querySelectorAll('#paymentMethodsWrap .pm-tile').forEach(function (t) {
        t.classList.toggle('sel', t.getAttribute('data-pm') === key);
      });
    ...
    function renderPaymentMethodDetail() {
      ...
      const m = window.selectedPaymentMethod;
      if (!detail) return;
      if (!m) { detail.style.display = 'none'; return; }
      detail.style.display = 'block';

      const title = document.getElementById('pmDetailTitle');
      if (title) title.textContent = 'Send Money to ' + (m.name || m.__key);

      const numEl = document.getElementById('pmDetailNumber');
      if (numEl) numEl.textContent = (m.number || '').trim() || 'Number not set yet';

      const img = document.getElementById('pmDetailQr');
      if (img) {
        if (m.qrImage) { img.src = m.qrImage; img.style.display = 'block'; }
        else { img.removeAttribute('src'); img.style.display = 'none'; }
      }

      const ins = document.getElementById('pmDetailInstructions');
      if (ins) {
        const globalIns = (window.storePaymentConfig && window.storePaymentConfig.instructions) || '';
        const text = (m.instructions || '').trim() || globalIns;
        ins.textContent = text;
        ins.style.display = text ? 'block' : 'none';
      }

      const btn = document.getElementById('confirmOrderBtn');
      if (btn) btn.innerHTML = '<i class="fas fa-check"></i> I paid via ' + escapeHtml(m.name || m.__key);

      const trxOff = m.trxRequired === false ||
        (window.storePaymentConfig && typeof window.storePaymentConfig.requireTrxId === 'boolean' &&
          window.storePaymentConfig.requireTrxId === false && typeof m.trxRequired !== 'boolean');
      const utrInput = document.getElementById('utrId');
      const optTag = document.getElementById('utrOptionalTag');
      if (utrInput) utrInput.required = !trxOff;
      if (optTag) optTag.textContent = trxOff ? '(optional)' : '';
    }

    function copyPaymentNumber() {
      const m = window.selectedPaymentMethod;
      if (!m || !m.number) { showNotification('No number to copy', 'error'); return; }
      const done = function () { showNotification(m.number + ' copied', 'success'); };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(m.number).then(done, function () { fallbackCopy(m.number, done); });
      } else {
        fallbackCopy(m.number, done);
      }
    }
    ...
    function fallbackCopy(text, done) {
      ...
      catch (e) { showNotification('Copy failed - select the number manually', 'error'); }
      document.body.removeChild(ta);
    }

    window.selectPaymentMethod = selectPaymentMethod;
    window.copyPaymentNumber = copyPaymentNumber;
    window.refreshPaymentMethods = loadStorePaymentMethods;
    window.getPaymentMethods = function () { return paymentMethods.slice(); };
...
<head>
  ...
  <style>
    ...
    *:focus {
    ...
    /* ===== Payment Methods ===== */
    .pm-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(270px, 1fr)); gap: 15px; }
    .pm-card { background: rgba(10, 14, 26, 0.85); border: 1px solid rgba(0, 255, 255, 0.2);
               border-radius: 12px; padding: 16px; position: relative; }
    .pm-card.off { opacity: .55; }
    .pm-head { display: flex; align-items: center; gap: 10px; margin-bottom: 12px; }
    .pm-ic { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center;
             justify-content: center; font-size: 17px; color: #fff; flex: 0 0 auto; }
    .pm-nm { font-weight: 700; color: #fff; }
    .pm-sub { font-size: 12px; color: #00ffff; letter-spacing: .04em; }
    .pm-rows { font-size: 12.5px; color: #a855f7; line-height: 1.9; margin-bottom: 12px;
               word-break: break-all; }
    .pm-row-label { color: #999; }
    .pm-btns { display: flex; gap: 8px; flex-wrap: wrap; }
    .pm-btn { border: 1px solid rgba(0,255,255,.35); background: rgba(0,255,255,.06); color: #00ffff;
              border-radius: 8px; padding: 6px 10px; font-size: 12px; cursor: pointer; }
    .pm-btn:hover { background: rgba(0,255,255,.16); }
    .pm-btn.del { border-color: rgba(255,0,110,.45); color: #ff4d6d; background: rgba(255,0,110,.06); }
    .pm-del-form { margin-top: 12px; display: flex; gap: 8px; align-items: center; }
    .pm-del-form input { flex: 1; background: rgba(0,0,0,.4); border: 1px solid #ff4d6d;
                         border-radius: 8px; padding: 8px; color: #fff; font-size: 12px; }
    .pm-empty { text-align: center; color: #999; padding: 30px 10px; font-size: 14px; }
    ...
    .switch input:checked + .slider { background: linear-gradient(135deg, #00ff88, #00cc6a); }
    .switch input:checked + .slider:before { transform: translateX(22px); background: #04150c; }
    .pm-ic-picker { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px; }
    .pm-ic-opt { width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center;
                 justify-content: center; cursor: pointer; border: 2px solid transparent;
                 background: rgba(0,0,0,.35); color: #cbd5e1; font-size: 16px; }
...
<body>
  ...
  <div class="drawer" id="drawer">
    ...
    <nav class="drawer-nav">
      ...
      <div class="drawer-nav-item" data-tab="paymentMethods">
        <span class="drawer-nav-icon"><i class="fas fa-wallet"></i></span>
  ...
  <div id="paymentMethodModal" class="modal">
    <div class="modal-content">
      <span class="modal-close" onclick="closePaymentMethodModal()">&times;</span>
      <h2 class="modal-title" id="paymentMethodModalTitle"><i class="fas fa-wallet"></i> Add Payment Method</h2>
      <div class="input-group">
        <label><i class="fas fa-tag"></i> Method Name</label>
        <input type="text" id="pmName" maxlength="22" placeholder="bKash">
      </div>
      <div class="input-group">
        <label><i class="fas fa-phone"></i> Payment Number / Wallet</label>
        <input type="text" id="pmNumber" maxlength="24" placeholder="01700000000 (Personal)">
      </div>
      <div class="input-group">
        <label><i class="fas fa-qrcode"></i> QR / Scan Image URL <span style="color:#999;font-size:11px;">(optional)</span></label>
        <input type="url" id="pmQr" placeholder="https://...">
        <img id="pmQrPreview" class="pm-preview" alt="QR preview" style="display:none;"
             onerror="this.style.display='none';">
      </div>
      <div class="input-group">
        <label><i class="fas fa-align-left"></i> Instructions <span style="color:#999;font-size:11px;">(optional)</span></label>
        <textarea id="pmInstructions" rows="3" maxlength="300" placeholder="Send Money to this number, then copy the Transaction ID"></textarea>
      </div>
      <div class="input-group">
        <label><i class="fas fa-icons"></i> Icon</label>
        <div class="pm-ic-picker" id="pmIconPicker"></div>
      </div>
      <div class="input-group">
        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
          <span class="switch"><input type="checkbox" id="pmEnabled" checked><span class="slider"></span></span>
          Enabled &mdash; visible to customers
        </label>
      </div>
      <div class="input-group">
        <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
          <span class="switch"><input type="checkbox" id="pmDefault"><span class="slider"></span></span>
          Default selection on checkout
        </label>
      </div>
      <div class="btn-group">
        <button class="btn btn-secondary" onclick="closePaymentMethodModal()">Cancel</button>
        <button class="btn" id="pmSaveBtn"><i class="fas fa-save"></i> Save Method</button>
      </div>
    </div>
  </div>
  ...
  <div id="adminDashboard" class="admin-container">
    ...
    <!-- Payment Methods Tab -->
    <div id="paymentMethodsTab" class="tab-content">
      <div class="card">
        <h2 class="card-title"><i class="fas fa-wallet"></i> Payment Methods</h2>
        <div id="paymentMethodsSuccess" style="display: none;" class="success-message"></div>
        <p style="color:#999;font-size:13px;line-height:1.6;margin-bottom:16px;">
          <i class="fas fa-info-circle"></i> Customers can pay using only the methods you keep
          <strong style="color:#00ff88;">Enabled</strong>. Number, QR and instructions update live on the
          checkout page. Minimum <strong style="color:#00ffff;">1</strong>, maximum
          <strong style="color:#00ffff;">8</strong> methods.
        </p>
        <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;margin-bottom:16px;">
          <label class="switch"><input type="checkbox" id="pmRequireNumber"><span class="slider"></span></label>
          <span style="color:#cbd5e1;font-size:13px;">Block saving a method without a payment number</span>
          <span style="flex:1"></span>
          <label class="switch"><input type="checkbox" id="pmRequireTrx" checked><span class="slider"></span></label>
          <span style="color:#cbd5e1;font-size:13px;">Customers must enter a Transaction ID</span>
        </div>
        <div id="paymentMethodsGrid" class="pm-grid"></div>
        <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;">
          <button class="btn" style="background:linear-gradient(135deg,#7c3aed,#a855f7);" id="addPaymentMethodBtn">
            <i class="fas fa-plus"></i> Add Payment Method
          </button>
          <button class="btn btn-secondary" id="restoreDefaultPaymentBtn">
            <i class="fas fa-rotate-left"></i> Restore bKash / Nagad / Rocket / Upay
          </button>
        </div>
      </div>
    </div>
  ...
  try {
    ...
    function createOrderCard(id, order) {
      ...
      card.innerHTML = `
        ...
        <div class="order-info">
          ...
          <div><span class="order-field">Payment:</span> <span class="order-value" style="color:#00d4ff;">${escapeHtml(order.paymentMethod || 'QR / Bank transfer')}</span></div>
          <div><span class="order-field">UTR / Trx ID:</span> <span class="order-value">${escapeHtml(order.userInput?.utrId || 'N/A')}</span></div>
    ...
    // Kept in the classic script (not the module block) so a blocked module
    // load cannot take the payment-method UI down with it.
    // ===== Payment Methods =====
    // Defaults live in the classic script so a blocked module load cannot break
    // the payment-method UI; the module block only mirrors them if present.
    if (!window.PAYMENT_ICONS) {
      window.PAYMENT_ICONS = [
        { cls: 'fa-mobile-alt', label: 'Mobile' },
        { cls: 'fa-wallet', label: 'Wallet' },
        { cls: 'fa-money-bill-wave', label: 'Cash' },
        { cls: 'fa-qrcode', label: 'QR' },
        { cls: 'fa-credit-card', label: 'Card' },
        { cls: 'fa-building-columns', label: 'Bank' }
      ];
    }

    const PM_PATH = 'meta/paymentMethods';
    let pmCache = {};
    let pmEditingKey = '';

    function pmIcons() {
      return (window.PAYMENT_ICONS && window.PAYMENT_ICONS.length)
        ? window.PAYMENT_ICONS : [{ cls: 'fa-wallet', label: 'Wallet' }];
    }

    function pmIconSel() {
      const el = document.querySelector('#pmIconPicker .pm-ic-opt.sel');
      return el ? el.getAttribute('data-ic') : pmIcons()[0].cls;
    }

    function pmBrandColor(key) {
      const map = { bkash: '#e2136e', nagad: '#f6921e', rocket: '#8c3494', upay: '#f47b20' };
      return map[String(key).toLowerCase()] || '#00ffff';
    }
    ...
    function pmKeyify(name) {
      return String(name || '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'method';
    }

    function pmList() {
      return Object.keys(pmCache).map(k => Object.assign({ __key: k }, pmCache[k] || {}))
        .sort((a, b) => pmSortKey(a) - pmSortKey(b));
    }

    async function loadPaymentMethods() {
      try {
        const snap = await window.dbGet(window.dbRef(window.db, PM_PATH));
        pmCache = snap.exists() ? (snap.val() || {}) : {};
      } catch (err) {
    ...
    function renderPaymentMethods() {
      ...
      const list = pmList();
      if (!list.length) {
        grid.innerHTML = '<div class="pm-empty"><i class="fas fa-wallet"></i> No payment method yet. Click "Add Payment Method" or restore the defaults.</div>';
        return;
      }
      grid.innerHTML = list.map(m => {
        ...
        const color = m.color || '#00ffff';
        const icon = m.iconCls || 'fa-wallet';
        const num = (m.number || '').trim();
        return '<div class="pm-card' + (on ? '' : ' off') + '" style="border-color:' + color + '33">' +
          '<div class="pm-head">' +
            '<div class="pm-ic" style="background:' + color + '22;color:' + color + '"><i class="fas ' + escapeHtml(icon) + '"></i></div>' +
            '<div style="flex:1;min-width:0">' +
              '<div class="pm-nm">' + escapeHtml(m.name || m.__key) + '</div>' +
              '<div class="pm-sub">' + (on ? 'ENABLED' : 'DISABLED') + (m.isDefault ? ' &middot; DEFAULT' : '') + '</div>' +
            '</div>' +
            '<label class="switch" title="Toggle on / off"><input type="checkbox" ' + (on ? 'checked' : '') +
              ' onchange="window.togglePaymentEnabled(\'' + m.__key + '\', this.checked)"><span class="slider"></span></label>' +
          '</div>' +
          '<div class="pm-rows">' +
            '<div><span class="pm-row-label">Number:</span> ' + (num ? escapeHtml(num) : '<span style="color:#ff9f43">not set</span>') + '</div>' +
            '<div><span class="pm-row-label">QR:</span> ' + (m.qrImage ? 'set' : '<span style="color:#ff9f43">not set</span>') + '</div>' +
            '<div><span class="pm-row-label">Trx ID:</span> ' +
              (m.trxRequired === false ? 'optional' :
               m.trxRequired === true ? 'required' :
               '<span style="color:#999">follows global</span>') + '</div>' +
          '</div>' +
          '<div class="pm-btns">' +
            '<button class="pm-btn" onclick="window.editPaymentMethod(\'' + m.__key + '\')"><i class="fas fa-pen"></i> Edit</button>' +
            '<button class="pm-btn del" onclick="window.deletePaymentMethod(\'' + m.__key + '\')"><i class="fas fa-trash"></i> Delete</button>' +
          '</div>' +
        '</div>';
      }).join('');
    }

    function renderPmIconPicker(selected) {
      const box = document.getElementById('pmIconPicker');
      if (!box) return;
      const want = selected || pmIcons()[0].cls;
      const list = pmIcons().some(o => o.cls === want) ? pmIcons() : [want].concat(pmIcons());
      box.innerHTML = list.map(o =>
        '<div class="pm-ic-opt' + (o.cls === want ? ' sel' : '') + '" data-ic="' + o.cls + '" ' +
        'title="' + o.label + '" onclick="window.pickPmIcon(this)">' +
        '<i class="fas ' + o.cls + '"></i></div>').join('');
    }

    window.pickPmIcon = function (el) {
      document.querySelectorAll('#pmIconPicker .pm-ic-opt').forEach(x => x.classList.remove('sel'));
      el.classList.add('sel');
    };

    window.addPaymentMethod = function () {
      pmEditingKey = '';
      document.getElementById('paymentMethodModalTitle').innerHTML =
        '<i class="fas fa-wallet"></i> Add Payment Method';
      ['pmName', 'pmNumber', 'pmQr', 'pmInstructions'].forEach(id => document.getElementById(id).value = '');
      document.getElementById('pmEnabled').checked = true;
      document.getElementById('pmDefault').checked = !pmList().length;
      document.getElementById('pmQrPreview').style.display = 'none';
      renderPmIconPicker(pmIcons()[0].cls);
      document.getElementById('paymentMethodModal').classList.add('active');
      document.body.style.overflow = 'hidden';
    };

    window.editPaymentMethod = async function (key) {
      const m = pmCache[key] || {};
      pmEditingKey = key;
      document.getElementById('paymentMethodModalTitle').innerHTML =
        '<i class="fas fa-pen"></i> Edit ' + escapeHtml(m.name || key);
      document.getElementById('pmName').value = m.name || '';
      document.getElementById('pmNumber').value = m.number || '';
      document.getElementById('pmQr').value = m.qrImage || '';
      document.getElementById('pmInstructions').value = m.instructions || '';
      document.getElementById('pmEnabled').checked = m.enabled !== false;
      document.getElementById('pmDefault').checked = !!m.isDefault;
      const prev = document.getElementById('pmQrPreview');
      if (m.qrImage) { prev.src = m.qrImage; prev.style.display = 'block'; } else { prev.style.display = 'none'; }
      renderPmIconPicker(m.iconCls || pmIcons()[0].cls);
      document.getElementById('paymentMethodModal').classList.add('active');
      document.body.style.overflow = 'hidden';
    };
    ...
    async function savePaymentMethod() {
      ...
      const iconEl = document.querySelector('#pmIconPicker .pm-ic-opt.sel');
      const enabled = document.getElementById('pmEnabled').checked;
      const isDefault = document.getElementById('pmDefault').checked;
      const requireNumber = document.getElementById('pmRequireNumber').checked;

      if (!name || name.length < 2) return showMessage('paymentMethodsSuccess', 'Name must be at least 2 characters', 'error');
      if (name.length > 22) return showMessage('paymentMethodsSuccess', 'Name is too long (max 22 characters)', 'error');
      if (!number && requireNumber) {
        return showMessage('paymentMethodsSuccess',
          'Add a payment number before enabling this method (or uncheck "Enabled")', 'error');
      }
      if (!document.querySelector('#pmIconPicker .pm-ic-opt')) {
        return showMessage('paymentMethodsSuccess', 'Icon picker unavailable, reload the page', 'error');
      }
      ...
      btn.innerHTML = '<span class="loading-spinner"></span>Saving...';
      try {
        const key = pmEditingKey || pmKeyify(name);
        const prevVal = pmCache[key] || {};
        const data = {
          id: key,
          name: name,
          number: number,
          qrImage: document.getElementById('pmQr').value.trim(),
          instructions: document.getElementById('pmInstructions').value.trim(),
          iconCls: pmIconSel(),
          color: prevVal.color || pmBrandColor(key),
          enabled: enabled,
          isDefault: isDefault,
          sortOrder: prevVal.sortOrder != null ? prevVal.sortOrder : pmList().length,
          updatedAt: new Date().toISOString()
        };
        await window.dbSet(window.dbRef(window.db, PM_PATH + '/' + key), data);
        if (isDefault) {
          for (const other of pmList()) {
            if (other.__key !== key && other.isDefault) {
              await window.dbUpdate(window.dbRef(window.db, PM_PATH + '/' + other.__key), { isDefault: false });
            }
          }
        }
        await loadPaymentMethods();
        showMessage('paymentMethodsSuccess',
          (pmEditingKey ? 'Updated ' : 'Added ') + name + ' \u2705', 'success');
        closePaymentMethodModal();
      } catch (error) {
        showMessage('paymentMethodsSuccess', 'Error: ' + error.message, 'error');
      } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-save"></i> Save Method';
      }
    }

    window.togglePaymentEnabled = async function (key, on) {
      try {
        const cur = pmCache[key] || {};
        if (on && !(cur.number || '').trim()) {
          await loadPaymentMethods();
          return showMessage('paymentMethodsSuccess',
            (cur.name || key) + ' has no payment number \u2014 edit it first', 'error');
        }
        if (on) {
          for (const other of pmList()) {
            if (other.__key !== key && other.isDefault) {
        ...
        await window.dbUpdate(window.dbRef(window.db, PM_PATH + '/' + key), {
          enabled: !!on,
          isDefault: !!on && (pmList().filter(m => m.enabled !== false).length === 0),
          updatedAt: new Date().toISOString()
        });
        await loadPaymentMethods();
        showMessage('paymentMethodsSuccess', (cur.name || key) + (on ? ' enabled' : ' disabled'), 'success');
      } catch (error) {
        showMessage('paymentMethodsSuccess', 'Error: ' + error.message, 'error');
      }
    };

    window.deletePaymentMethod = async function (key) {
      const nm = (pmCache[key] || {}).name || key;
      if (!confirm('Delete "' + nm + '"? Customers will lose this option immediately.')) return;
      try {
        await window.dbRemove(window.dbRef(window.db, PM_PATH + '/' + key));
        await loadPaymentMethods();
        showMessage('paymentMethodsSuccess', nm + ' deleted', 'success');
      } catch (error) {
        showMessage('paymentMethodsSuccess', 'Error: ' + error.message, 'error');
      }
    };
    ...
    async function restoreDefaultPaymentMethods() {
      try {
        const existing = pmCache || {};
        let added = 0;
        const DEFS = {
          bkash: { name: 'bKash', iconCls: 'fa-mobile-alt', enabled: true, default: true },
          nagad: { name: 'Nagad', iconCls: 'fa-mobile-alt', enabled: false, default: false },
          rocket: { name: 'Rocket', iconCls: 'fa-wallet', enabled: false, default: false },
          upay: { name: 'Upay', iconCls: 'fa-money-bill-wave', enabled: false, default: false }
        };
        for (const [key, def] of Object.entries(DEFS)) {
          const cur = existing[key];
          if (cur && (cur.name || '').trim() && (cur.number || '').trim()) continue;
          await window.dbSet(window.dbRef(window.db, PM_PATH + '/' + key), Object.assign({
            id: key,
            number: '',
            qrImage: '',
            instructions: '',
            sortOrder: Object.keys(existing).length + added,
            updatedAt: new Date().toISOString()
          }, cur || {}, {
            name: def.name, iconCls: def.iconCls, color: pmBrandColor(key),
            enabled: def.enabled, isDefault: def.default
          }));
          added++;
        }
        await loadPaymentMethods();
        showMessage('paymentMethodsSuccess', added ? 'Restored/updated ' + added + ' default method(s) \u2014 now add the numbers' : 'Defaults already present', 'success');
      } catch (error) {
        showMessage('paymentMethodsSuccess', 'Error: ' + error.message, 'error');
      }
    }

    document.getElementById('addPaymentMethodBtn')?.addEventListener('click', () => window.addPaymentMethod());
    document.getElementById('pmSaveBtn')?.addEventListener('click', savePaymentMethod);
    document.getElementById('restoreDefaultPaymentBtn')?.addEventListener('click', restoreDefaultPaymentMethods);
    document.getElementById('pmQr')?.addEventListener('input', function () {
      const url = this.value.trim();
      const img = document.getElementById('pmQrPreview');
      if (url) { img.src = url; img.style.display = 'block'; } else { img.style.display = 'none'; }
    });


    // ===== global payment config (meta/paymentConfig) =====
    async function loadPaymentConfig() {
      try {
        const snap = await window.dbGet(window.dbRef(window.db, 'meta/paymentConfig'));
        const d = snap.exists() ? (snap.val() || {}) : {};
        const t = document.getElementById('pmRequireTrx');
        const r = document.getElementById('pmRequireNumber');
        if (t) t.checked = d.requireTrxId !== false;
        if (r) r.checked = !!d.requireNumber;
      } catch (error) {
    ...
    async function savePaymentConfig() {
      ...
      const r = document.getElementById('pmRequireNumber');
      try {
        await window.dbSet(window.dbRef(window.db, 'meta/paymentConfig'), {
          requireTrxId: t ? t.checked : true,
          requireNumber: r ? r.checked : false,
          updatedAt: new Date().toISOString()
        });
        showMessage('paymentMethodsSuccess', 'Payment rules saved', 'success');
      } catch (error) {
        showMessage('paymentMethodsSuccess', 'Error: ' + error.message, 'error');
      }
    }

    document.getElementById('pmRequireTrx')?.addEventListener('change', savePaymentConfig);
    document.getElementById('pmRequireNumber')?.addEventListener('change', savePaymentConfig);
    window.loadPaymentConfig = loadPaymentConfig;
    window.loadPaymentMethods = loadPaymentMethods;
    window.pmList = pmList;
...
<title>Premium Store — pick a page</title>
<style>
  * { box-sizing: border-box; }
  body {
  ...
  .sub { color: #94a3b8; margin: 0 0 1.75rem; font-size: .95rem; }
  .grid { display: grid; gap: 1rem; grid-template-columns: 1fr 1fr; }
  @media (max-width: 520px) { .grid { grid-template-columns: 1fr; } }
  a.card {
    display: block; text-decoration: none; color: inherit;
    background: #111a2c; border: 1px solid #23304a; border-radius: 14px;
    padding: 1.35rem 1.4rem; transition: .16s ease;
  }
  ...
  .name { font-size: 1.05rem; font-weight: 600; margin-bottom: .3rem; }
  .meta { font-size: .82rem; color: #94a3b8; }
  .note { margin-top: 1.5rem; font-size: .82rem; color: #64748b;
          border-left: 2px solid #334155; padding-left: .8rem; line-height: 1.55; }
  .ok { color: #4ade80; }
</style>
</head>
<body>
  <div class="wrap">
    <h1>Premium Store</h1>
    <p class="sub">Served by <code>bot.py</code> — one file, everything inside — <span class="ok">running</span>, port 8000</p>
    <div class="grid">
      <a class="card" href="user.html">
        <div class="tag">Customer side</div>
        <div class="name">Shop</div>
        <div class="meta">user.html &middot; bKash/Nagad/Rocket/Upay picker</div>
      </a>
      <a class="card" href="health">
        <div class="tag">Deploy check</div>
        <div class="name">/health</div>
        <div class="meta">JSON status from bot.py</div>
      </a>
      <a class="card" href="admin.html">
        <div class="tag">Owner side</div>
        <div class="name">Admin Panel v3.0</div>
        <div class="meta">admin.html &middot; Payment Methods tab</div>
      </a>
    </div>
    <p class="note">
      <strong>New:</strong> Payment Methods live in the admin drawer
      (<em>Payment Methods</em> tab) &rarr; stored at <code>meta/paymentMethods</code>.
      Deploy: <code>python bot.py</code> (Railway sets <code>$PORT</code>).
      Enable/disable, edit number &amp; QR, or delete per method; the checkout page
      follows instantly. Both pages read/write through Firebase Realtime Database
      (<code>cipher-pro-store</code>). An anonymous read comes back
      <strong>401 Permission denied</strong>, so the UI renders but the
      catalogue stays empty until you sign in through the page's own auth flow.
    </p>
  </div>
</body>
</html>
